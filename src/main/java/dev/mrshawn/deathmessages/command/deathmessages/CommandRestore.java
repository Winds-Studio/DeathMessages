...
public class CommandRestore extends DeathMessagesCommand {
	...
	public void onCommand(CommandSender sender, String[] args) {
		...
		if (args.length <= 1) {
			sender.sendMessage(Assets.convertFromLegacy(Assets.formatMessage("Commands.DeathMessages.Sub-Commands.Restore.Usage")));
		} else {
			String code = args[0];
			boolean excludeUserData = Boolean.parseBoolean(args[1]);
			if (ConfigManager.getInstance().restore(code, excludeUserData)) {
				sender.sendMessage(Assets.convertFromLegacy(Assets.formatMessage("Commands.DeathMessages.Sub-Commands.Restore.Restored")
						.replaceAll("%backup-code%", code)));
			} else {
				sender.sendMessage(Assets.convertFromLegacy(Assets.formatMessage("Commands.DeathMessages.Sub-Commands.Restore.Backup-Not-Found")));
			}
		}
	}
}
