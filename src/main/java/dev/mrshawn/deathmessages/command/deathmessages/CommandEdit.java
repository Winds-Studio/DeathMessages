...
public class CommandEdit extends DeathMessagesCommand {
	...
	public void onCommand(CommandSender sender, String[] args) {
		...
		if (args.length <= 3) {
			sender.sendMessage(Assets.convertLegacy(Assets.formatMessage("Commands.DeathMessages.Sub-Commands.Edit.Usage")));
		} else {
			if (args[0].equalsIgnoreCase("player")) {
				...
				if (!exists) {
					sender.sendMessage(Assets.convertLegacy(Assets.formatMessage("Commands.DeathMessages.Sub-Commands.Edit.Invalid-Mob-Type")));
					return;
				}
				if (!DamageTypes.getFriendlyNames().contains(damageType)) {
					sender.sendMessage(Assets.convertLegacy(Assets.formatMessage("Commands.DeathMessages.Sub-Commands.Edit.Invalid-Damage-Type")));
					return;
				}
				if (args[4].equalsIgnoreCase("add")) {
					if (args[2].equalsIgnoreCase("solo")) {
						sender.sendMessage(Assets.convertLegacy(Assets.formatMessage("Commands.DeathMessages.Sub-Commands.Edit.Adding-Start")));
						...
						sender.sendMessage(Assets.convertLegacy(Assets.formatMessage("Commands.DeathMessages.Sub-Commands.Edit.Invalid-Arguments")));
					}
					...
					if (!Assets.isNumeric(args[5])) {
						sender.sendMessage(Assets.convertLegacy(Assets.formatMessage("Commands.DeathMessages.Sub-Commands.Edit.Invalid-Placeholder")));
						return;
					}
					...
					if (args[2].equalsIgnoreCase("solo")) {
						...
						if (list.get(placeholder) == null) {
							sender.sendMessage(Assets.convertLegacy(Assets.formatMessage("Commands.DeathMessages.Sub-Commands.Edit.Invalid-Selection")));
							return;
						}
						sender.sendMessage(Assets.convertLegacy(Assets.formatMessage("Commands.DeathMessages.Sub-Commands.Edit.Removed-Message").replaceAll("%message%", list.get(placeholder))));
					...
					if (args[2].equalsIgnoreCase("solo")) {
						...
						for (String messages : list) {
							sender.sendMessage(Assets.convertLegacy("[" + placeholder + "] " + Assets.formatString(messages)));
							placeholder++;
						}
					} else if (args[2].equalsIgnoreCase("gang")) {
						List<String> list = PlayerDeathMessages.getInstance().getConfig().getStringList("Mobs." + mobName + ".Gang." + damageType);
