...
public class EntityDamageByEntity implements Listener {
    ...
    public void entityDamageByEntity(EntityDamageByEntityEvent e) {
        ...
        EntityUtil.loadCrystalDamager(e.getEntity(), e.getDamager());
        ...
        if (e.getEntity() instanceof Player) {
            ...
            if (playerCtx != null) {
                if (e.getCause().equals(EntityDamageEvent.DamageCause.ENTITY_EXPLOSION)) {
                    Entity lastCrystalDamager = EntityUtil.crystalDeathContext.get(e.getDamager().getUniqueId());

                    switch (e.getDamager()) {
                        case EnderCrystal enderCrystal when lastCrystalDamager != null -> {
                            playerCtx.setLastEntityDamager(lastCrystalDamager);
                            playerCtx.setLastExplosiveEntity(e.getDamager());
                        }
                        case TNTPrimed tnt -> { // For <= 1.20.2, because TNT explosion became BLOCK_EXPLOSION since 1.20.3
                            if (tnt.getSource() instanceof LivingEntity) {
                                playerCtx.setLastEntityDamager(tnt.getSource());
                            }
                            playerCtx.setLastExplosiveEntity(e.getDamager());
                        }
                        case Firework firework when PlatformUtil.isNewerAndEqual(16, 0) -> { // Firework extends Entity under <= 1.15
                            try {
                    ...
                    if (entityCtx != null) {
                        if (e.getCause().equals(EntityDamageEvent.DamageCause.ENTITY_EXPLOSION)) {
                            ...
                            switch (e.getDamager()) {
                                ...
                                case Firework firework when PlatformUtil.isNewerAndEqual(16, 0) -> {
                                    try {
